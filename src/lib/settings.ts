...
//
// Stored as a singleton row (id = 1) in ecom.dev_notice.
// ---------------------------------------------------------------------------
...
/**
 * Read the dev-notice settings. Never throws — if the ecom.dev_notice table
  ...
  try {
    const row = await queryOne<DevNotice>(
      `select enabled, title, message,
         ...
         from ecom.dev_notice where id = 1`,
    );
    return row ?? DEFAULTS;
  } catch {
    return DEFAULTS;
...
export async function setDevNotice(
  ...
  await query(
    `insert into ecom.dev_notice (id, enabled, title, message, updated_by, updated_at)
...
// from the dev-notice modal). Singleton row (id = 1) in ecom.announcement.
// ---------------------------------------------------------------------------
  ...
  try {
    const row = await queryOne<Announcement>(
      `select enabled, message, nullif(link,'') as link,
              updated_at as "updatedAt", updated_by as "updatedBy"
         from ecom.announcement where id = 1`,
    );
...
export async function setAnnouncement(
  ...
  await query(
    `insert into ecom.announcement (id, enabled, message, link, updated_by, updated_at)
       values (1, $1, $2, $3, $4, now())
     on conflict (id) do update
       set enabled    = excluded.enabled,
           message    = excluded.message,
  ...
  try {
    const row = await queryOne<{
      ...
      `select enabled,title,ends_at as "endsAt",
              updated_at as "updatedAt",updated_by as "updatedBy"
         from ecom.home_promotion where id=1`,
    );
    return row
      ? {
          ...row,
...
export async function setHomePromotion(
  ...
  await query(
    `insert into ecom.home_promotion(id,enabled,title,ends_at,updated_by,updated_at)
       values(1,$1,$2,$3,$4,now())
     on conflict(id) do update
...
// know where to send the money. Singleton (id = 1) in ecom.bank_transfer.
// ---------------------------------------------------------------------------
  ...
  try {
    const row = await queryOne<BankTransfer>(
      `select bank_name as "bankName", account_name as "accountName",
         ...
         from ecom.bank_transfer where id = 1`,
    );
...
export async function setBankQr(url: string | null, by?: string): Promise<void> {
  await query(
    `insert into ecom.bank_transfer (id, qr_url, updated_by, updated_at)
       values (1, $1, $2, now())
     on conflict (id) do update
...
export async function setBankTransfer(
  ...
  await query(
    `insert into ecom.bank_transfer (id, bank_name, account_name, account_no, note, updated_by, updated_at)
  ...
  try {
    const row = await queryOne<{
      ...
      `select test_mode as "testMode", test_amount::text as "testAmount",
              updated_at as "updatedAt", updated_by as "updatedBy"
         from ecom.onepay_config where id = 1`,
    );
    return row
...
export async function setOnepayRuntimeConfig(
  ...
  await query(
    `insert into ecom.onepay_config (id, test_mode, test_amount, updated_by, updated_at)
       values (1,$1,$2,$3,now())
     on conflict (id) do update
...
export async function getCodEnabled(): Promise<boolean> {
  try {
    const row = await queryOne<{ codEnabled: boolean }>(
      `select cod_enabled as "codEnabled" from ecom.onepay_config where id = 1`,
    );
...
export async function setCodEnabled(enabled: boolean, by?: string): Promise<void> {
  await query(
    `insert into ecom.onepay_config (id, cod_enabled, updated_by, updated_at)
       values (1,$1,$2,now())
     on conflict (id) do update
