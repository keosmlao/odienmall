...
export default async function RedemptionDetailPage({
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <PageHeader title="ລາຍລະອຽດການແລກ" back={{ href: "/admin/rewards/redemptions", label: "ກັບໄປລາຍການ" }} />

      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-black text-slate-900">{r.rewardName}</h1>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
          <div><dt className="text-slate-500">ແຕ້ມທີ່ໃຊ້</dt><dd className="font-bold text-orange-600">{r.pointsSpent.toLocaleString()}</dd></div>
          <div><dt className="text-slate-500">ຂອງລາງວັນ</dt><dd className="text-slate-800">{r.freeQty != null && r.unitCode ? `${r.freeQty} ${r.unitCode}` : "—"}</dd></div>
          <div><dt className="text-slate-500">ລະຫັດສິນຄ້າ</dt><dd className="text-slate-800">{r.icCode ?? "—"}</dd></div>
          <div><dt className="text-slate-500">ໃບຂໍເບີກ (SML)</dt><dd className="text-slate-800">{r.smlDocNo ?? "—"}</dd></div>
        </dl>
        {r.note && <p className="mt-3 rounded-lg bg-slate-100 px-3 py-2 text-xs text-slate-700">{r.note}</p>}
      </div>
