    ...
    >
      ...
      <div className="relative w-full lg:min-w-[240px] lg:flex-1">
        ...
        <input
          ...
          className="adm-focus w-full rounded-lg border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm font-semibold text-slate-700 transition focus:border-orange-400 focus:bg-white placeholder:text-slate-500"
        />
      </div>
      ...
      <div className="grid w-full grid-cols-[auto_1fr_auto_1fr] items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 sm:flex sm:gap-2 sm:px-3 lg:w-auto lg:min-w-[280px]">
        <span className="pr-1 text-xs font-bold uppercase tracking-wider text-slate-500">ວັນທີ</span>
        <input
          type="date"
          value={f}
      ...
      <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:items-center">
        <button
          type="submit"
          className="adm-focus rounded-lg bg-white px-5 py-2.5 text-sm font-black text-white transition hover:bg-orange-600 cursor-pointer"
        >
          ກັ່ນຕອງ
        </button>
      ...
      >
        <svg viewBox="0 0 24 24" className="h-4.5 w-4.5 text-slate-500 transition-colors" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
